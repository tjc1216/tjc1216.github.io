...
$.ajax({
  ...
  data: {
    key: 'XKGBZ-75XLT-5ZTX2-V73H7-OPZIJ-LFBJ4',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(121.46811527525074, 31.1475561568373, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("03/23/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by tjc1216 %c 你正在访问TJCの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| TJC") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('TJC的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
